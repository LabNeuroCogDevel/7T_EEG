...
fooof <- read.csv('/Volumes/Hera/Projects/7TBrainMech/scripts/eeg/Shane/fooof/Results/allSubjectsFooofMeasures_20230516.csv')
...
# 9.0 FOOOF vs Beh ----
  ...
  fooofBehavior <- merge(fooof_regional_age, m_clean, by = c("Subject", "visitno"))
...
## 9.3 FOOOF Behavior Mediation ----
  ### Exponent and Latency ----
  
  mediationMatrix <- fooofBehavior %>% dplyr::select(mgsLatency, Exponent, Condition, Region, luna, visitno, age.x) 
  mediationMatrix <- mediationMatrix[complete.cases(mediationMatrix), ]
  
  #the effect of age on exponent (c)
  model.0 <- lme4::lmer(mgsLatency ~ age.x + (1|luna), data = mediationMatrix)
  print(car::Anova(model.0))
  print(summary(model.0))
  summ(model.0)
  #anova(model.00, model.0) # significant improvement in fit when you include age 
  ...
  #the effect of age on imbalnce  (a)
  model.M <- lme4::lmer(Exponent ~ age.x + Region + (1|luna) + Condition, data = mediationMatrix )
  print(car::Anova(model.M))
  print(summary(model.M))
  summ(model.M)
  
  #the effect of MRS imbalance on exponent (b)
  model.Y <- lme4::lmer(mgsLatency ~ Exponent + age.x + (1|luna), data = mediationMatrix)
  print(car::Anova(model.Y))
  print(summary(model.Y))
  summ(model.Y)
  
  
  results <- mediation::mediate(model.M, model.Y, treat = "age.x", mediator = "Exponent", boot = FALSE, sims = 1000)
  (summary(results))
  
  
  ### Offset and Latency ----
  
  mediationMatrix <- fooofBehavior %>% dplyr::select(mgsLatency, Offset, Condition, Region, luna, visitno, age.x) 
  mediationMatrix <- mediationMatrix[complete.cases(mediationMatrix), ]
  ...
  #the effect of age on imbalnce  (a)
  model.M <- lme4::lmer(Offset ~ age.x + Region + (1|luna) + Condition, data = mediationMatrix )
  print(car::Anova(model.M))
  print(summary(model.M))
  summ(model.M)
  
  #the effect of MRS imbalance on exponent (b)
  model.Y <- lme4::lmer(mgsLatency ~ Offset + age.x + (1|luna), data = mediationMatrix)
  print(car::Anova(model.Y))
  print(summary(model.Y))
  summ(model.Y)
  
  
  results <- mediation::mediate(model.M, model.Y, treat = "age.x", mediator = "Offset", boot = FALSE, sims = 1000)
  (summary(results))
