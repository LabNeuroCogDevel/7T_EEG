...
ggplot(data=all_data, aes(x=Age, y=GABA.Cre, color=epoch)) + geom_point() + stat_smooth(method='loess')
ggplot(data=all_data, aes(x=Age, y=Glu, color=epoch)) + geom_point() + stat_smooth(method='loess')
ggplot(data=all_data, aes(x=logPower, y=GABA, color=epoch)) + geom_point() + stat_smooth(method='loess')
...
write.csv(dontHave, file="/Volumes/Hera/Projects/7TBrainMech/scripts/eeg/Shane/Results/Power_Analysis/donthave.csv")
