...
knitr::opts_chunk$set(echo = TRUE, warning=FALSE, dev='png', dpi=150, fig.height=4, fig.width = 6, fig.path='/Users/shanemckeon/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Lab/Projects/FOOOF/paperFigs/')
```
...
fooofMRSbehavior <- read.csv('/Users/shanemckeon/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Lab/Projects/FOOOF/R_Markdown/allSubjectsDLPFCfooofMRSBehaviorMeasures_20231031.csv')
fooofLong <- read.csv('/Users/shanemckeon/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Lab/Projects/FOOOF/R_Markdown/allSubjectsDLPFCfooofMeasures_20231031.csv')
MRSlong <- read.csv("/Users/shanemckeon/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Lab/Projects/FOOOF/R_Markdown/allSubjectsDLPFCMRSMeasures_20231031.csv")
fooofMRS<- read.csv("/Users/shanemckeon/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Lab/Projects/FOOOF/R_Markdown/allSubjectsDLPFCfooofMRSMeasures_20231031.csv")
...
#                     geom_line(aes(group=interaction(luna,Region,Condition), linetype = Condition, shape =Region), alpha = 0.2) + 
#                     geom_point(aes(shape=Region, color = ageGroup), size =2,alpha=.8) + 
#                     geom_smooth(aes(group = 1, color = "black"),method=mgcv::"gam", formula = y ~ s(x, k =, 3, fx = T),alpha=0.4,size=1)) + 
#   scale_color_manual(values=c("#E3B23C", "plum4", "#2E041F","black"))+plot.margin = margin(0.5,0.5,0.5,0.5,"cm")+
#   xlab("Offset") + ylab("Exponent") +
#   annotate("text", x = 1, y = 0.2, label = expression(rho ~" = 0.75, p < 2.2e-16"), size = 10, hjust =0)+
#   theme(text = element_text(size = 25), 
#         legend.position = "none", aspect.ratio = 1,axis.title.y = element_text(vjust = 2), 
#         axis.title.x = element_text(vjust = -2)) 



psd <- read.csv('/Users/shanemckeon/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Lab/Projects/FOOOF/R_Markdown/PSDtable.csv')
freqs <- read.csv('/Users/shanemckeon/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Lab/Projects/FOOOF/R_Markdown/freqs.csv')
...
# 
# psd_byBand <- psd_long_filtered %>% filter(!is.na(Band)) %>% 
#   group_by(Subject, age, Band) %>% 
#   dplyr::summarize(meanPower = mean(Power, na.rm=T))
# 
# ggplot(data = psd_byBand, aes(x = age, y = meanPower, color = Band)) +
#   geom_point(alpha=0.1) +
#   stat_smooth(aes(fill=Band), method='gam', formula=y~s(x,k=4), alpha=0.1) +
...
### Create derivatives ----
# idx <- which(!is.na(fooofMRS$Exponent))
# model <- lmer(Exponent ~ age + Condition + Region + (1|luna), data = fooofMRS[idx, ])
# fooofMRS$ExponentAgeRegressedOut <- NA
# fooofMRS[idx,]$ExponentAgeRegressedOut <- residuals(model)
# 
# 
# lunaize(ggplot(data = fooofMRS,
#                aes(x = GluGABAimbalanceABS, y = ExponentAgeRegressedOut, by = luna, color = ageGroup,shape=Region)) + 
...
#   scale_color_manual(values=c("#E3B23C", "plum4", "#2E041F","black")) + 
#   xlab("Glu GABA Asymmetry") + ylab("ExponentAgeRegressedOut")
...
# summary(lmerTest::lmer(Exponent ~ sqrt(GluGABAimbalanceABS) + age + Region + Condition + (1|luna), 
#                        data = fooofMRS))
