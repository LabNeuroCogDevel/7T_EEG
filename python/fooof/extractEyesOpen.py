...
outpath = '/Volumes/Hera/Projects/7TBrainMech/scripts/eeg/Shane/Results/Entropy/'

# loop to find the whole spectrum for eyes open
for fname in files:
    sub = fname[121:135] + '_eyesOpenRestData.set'
    ...
    closed_idx = np.where((raw.annotations.description != '16130') & (raw.annotations.description != '15262') & (
                raw.annotations.description != '1'))[0]  #  find indices where the trigger DOES NOT equal 16129, 15361, 0(eyes closed) or 16130, 15262, 1 (eyes open)
...
mne.export.export_raw(savefile, appendedRawSegments_editBoundary, fmt='eeglab', physical_range='auto', add_ch_type=False, overwrite=True, verbose=None)