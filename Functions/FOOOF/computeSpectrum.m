...
for i = 1:n
    ...
    freqs = freqs;
    psdDLPFC = psd(:,channelValues);
    psdAvgDLPFC = mean(psdDLPFC,2);
    plot(freqs,psdAvgDLPFC)
    
  
    subjectPSDs{i} = psdAvgDLPFC;
    subjects{i} = subject;
    
end

subjectsTable = cell2table(subjects');
subjectsTable.Properties.VariableNames{1} = 'Subject';

PSDtable = table();
PSDtable.Subject = subjects';
PSDtable.PSDs = subjectPSDs';
...
writetable(mergedTable, '/Volumes/Hera/Projects/7TBrainMech/scripts/eeg/Shane/Results/FOOOF/PSDtable.csv');
